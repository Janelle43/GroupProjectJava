...
public class MovieCatalogService {
    ...
    public CatalogItem getMovie(Long movieId) {
        ...
        Rating rating = restTemplate.getForObject("http://localhost:8082/movie/ratings/" + movieId, Rating.class);
    ...
    public Account getAccount(Long userId) {
        ...
        Account account = restTemplate.getForObject("http://localhost:8083/user/account/" + userId, Account.class);

        AccountBooking booking = restTemplate.getForObject("http://localhost:8084/bookingData/" + userId, AccountBooking.class);

        AccountRecommendation recommendation = restTemplate.getForObject("http://localhost:8085/recommendationsData/" + userId, AccountRecommendation.class);

        for (Booking list : booking.getBookings()) {

            Movie movie = restTemplate.getForObject("http://localhost:8081/movies/" + list.getMovie_id(), Movie.class);
